...
watch (
  ...
  newMarathon => {
    ...
    if (newMarathon === 'half') {
      if (map.getLayer('fullMarathon')) {
        map.removeLayer('fullMarathon');
      }
      map.addLayer($mapConfig.mapLayers.halfMarathon, 'cyclomediaRecordings')
    } else if (newMarathon === 'full') {
      if (map.getLayer('halfMarathon')) {
        map.removeLayer('halfMarathon');
      }
      map.addLayer($mapConfig.mapLayers.fullMarathon, 'cyclomediaRecordings')
    }
  }
)
...
const center = computed(() => {
  if (GeocodeStore.aisData.features && GeocodeStore.aisData.features[0]) {
    ...
    return $mapConfig.cityCenterCoords;
  }
})
...
onMounted(async () => {
  ...
  map = new maplibregl.Map({
    container: 'map',
    style: $mapConfig['pwdDrawnMapStyle'],
    center: center.value,
    zoom: zoom.value,
    minZoom: 6,
    maxZoom: 22,
...
const toggleImagery = () => {
  ...
  if (!MapStore.imageryOn) {
    ...
    if (!route.params.topic) {
      // map.setStyle($mapConfig['pwdDrawnMapStyle']);
